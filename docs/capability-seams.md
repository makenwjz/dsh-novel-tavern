...
| `ctx.novel` | `core` | [`novel`](../packages/novel/novel) | - | `tool-novel` | - | The world engine, plot vow ledger, creative decisions, and chapter knowledge control live in one SQLite-backed store; the dsh-novel-tools family reads and writes it through ctx.novel. |
