...
| `@deepseek-ai/dsh-novel-tools` | `chapter_info`, `decision_list`, `decision_record`, `lore_context`, `lore_list`, `lore_register`, `manuscript_read`, `manuscript_write`, `nb_export`, `nb_import`, `novel_lint`, `plot_list`, `plot_scene`, `plot_story`, `plot_thread`, `vow_abandon`, `vow_advance`, `vow_list`, `vow_payoff`, `vow_plant`, `world_event`, `world_history`, `world_state`, `world_subject` | `ctx.tools`, `ctx.novel (mount the dsh-novel-bundle composition)` | `tool/call`, `tool/result` | - | The 11 store tools fail loud with the missing-runtime error unless the dsh-novel-bundle composition mounts ctx.novel; novel_lint is pure text analysis and works without it. Story times travel in display form (±YYYY.MM.DD). |
...
Insert or update the knowledge-control ledger for one chapter: what the reader learns, what the protagonist learns, and what the draft must conceal or may hint at. Omitted knowledge fields keep their previous values.
...
List recorded creative decisions, newest first, with options, choice, and rationale.
...
Record a creative decision: the context, the options weighed (with pros and cons), and its open or decided status.
...
Read the knowledge layer of one subject: the omniscient canon (known to everyone, never to be contradicted) plus the entries scoped to that subject. With no subject, only the omniscient canon is returned — the author's bible view.
...
```json
{
  ...
  "properties": {
    "subjectId": {
      "type": "string",
      "description": "Optional subject whose knowledge is exposed, as an id returned by `world_subject`."
    }
  }
}
```
...
List the canon lorebook entries, optionally filtered by category, subject, or knowledge layer, in id order.
...
```json
{
  ...
  "properties": {
    "category": {
      "type": "string",
      "description": "Optional category filter: world, character, location, faction, item, event, system, instruction, or note."
    },
    "subjectId": {
      "type": "string",
      "description": "Optional subject filter; returns entries scoped to that subject."
    },
    "omniscient": {
      "type": "boolean",
      "description": "Optional layer filter: true for omniscient canon, false for character-scoped entries."
    }
  }
}
```
...
Register or update one canon lorebook entry (the story bible). Omniscient entries are canon known to everyone and must never be contradicted; character-scoped entries (omniscient false) are known only to the given subject. Omit `id` to create; provide it to update.
...
```json
{
  ...
  "properties": {
    "id": {
      "type": "string",
      "description": "Existing entry id to update; omit to create."
    },
    "category": {
      "type": "string",
      "description": "Entry category: world, character, location, faction, item, event, system, instruction, or note."
    },
    "title": {
      "type": "string",
      "description": "Display name of the entry; trimmed to a non-empty string."
    },
    "content": {
      "type": "string",
      "description": "The canon text; trimmed to a non-empty string."
    },
    "omniscient": {
      "type": "boolean",
      "description": "Whether this is omniscient canon (default true). Set false to scope the entry to `subjectId`."
    },
    "subjectId": {
      "type": "string",
      "description": "The subject that knows this entry; required when `omniscient` is false."
    }
  },
  "required": [
    "category",
...
Read one chapter's prose draft, or list every draft in chapter order when no number is given.
...
```json
{
  ...
  "properties": {
    "number": {
      "type": "number",
      "description": "Optional 1-based chapter number to read."
    }
  }
}
```
...
Upsert one chapter's prose draft in the manuscript ledger. The chapter number is the stable key; omitted content keeps the previous draft. Chapter knowledge control lives in `chapter_info`.
...
```json
{
  ...
  "properties": {
    ...
    "title": {
      "type": "string",
      "description": "Non-empty chapter title."
    },
    "content": {
      "type": "string",
      "description": "The chapter's prose."
    }
  },
  "required": [
    "number",
...
Export the novel workspace into one neuro-book-shaped project: canon lore becomes `lorebook/<category>/` markdown, character-scoped lore is archived under `reference/dsh-scoped-lore/`, and the world engine becomes WorldSubject / WorldSlice / WorldPatch rows in `.nbook/project.sqlite` plus a portable JSONL mirror.
...
```json
{
  ...
  "properties": {
    "root": {
      "type": "string",
      "description": "Absolute path of the target project root; it may be empty. Exporting into a project that already holds World rows at the same instants fails loud."
    }
  },
  "required": [
    "root"
...
```json
{
  ...
  "properties": {
    "root": {
      "type": "string",
      "description": "Absolute path of the neuro-book project root (the directory holding `lorebook/` and `.nbook/`)."
    }
  },
  "required": [
    "root"
...
Run the style linter over a manuscript excerpt: mechanically detectable prose habits (banned adverbs, stacked punctuation, told emotion, clichés, and similar) reported per line. Run this on a draft before or after revising.
...
Read the whole plot tree: stories, their threads, and every scene with its anchored story time, location, status, subjects, and the vows attached to it (each with its current status and an `overdue` hint when a not-yet-settled vow was planted before the scene's story time).
...
Create or update one plot scene inside a thread. A scene is the smallest story unit; anchor it to a story time, location, and the subjects present, and attach the vows it should advance or pay off. Omit `id` to create a scene (requires `threadId`); provide `id` to update the existing scene.
...
```json
{
  ...
  "properties": {
    "id": {
      "type": "string",
      "description": "Existing scene id to update; omit to create a new scene."
    },
    "threadId": {
      "type": "string",
      "description": "The owning thread id returned by `plot_thread` (required when creating)."
    },
    "title": {
      "type": "string",
      "description": "Display name of the scene; trimmed to a non-empty string. Required when creating."
    },
    "summary": {
      "type": "string",
      "description": "Free-form summary of what happens in the scene."
    },
    "at": {
      "type": "string",
      "description": "Optional story-time anchor, as `±YYYY.MM.DD`; the world state is queryable at it."
    },
    "location": {
      "type": "string",
    ...
    "subjectIds": {
      "type": "array",
      "description": "Subjects present in the scene, as ids returned by `world_subject`.",
      "items": {
        "type": "string"
      }
    },
    "vowIds": {
      "type": "array",
      "description": "Vows this scene is expected to advance or pay off, as ids returned by `vow_plant`.",
      "items": {
        "type": "string"
      }
    },
    "position": {
      "type": "number",
      "description": "Ordering position within the thread; lower first."
    },
    "status": {
      "type": "string",
      "description": "Writing state: `planned`, `writing`, or `written`."
    }
  }
}
```
...
Create one plot story: the causal spine that plot threads belong to.
...
```json
{
  ...
  "properties": {
    "title": {
      "type": "string",
      "description": "Short display name of the story; trimmed to a non-empty string."
    },
    "summary": {
      "type": "string",
      "description": "Free-form summary of the story."
    }
  },
  "required": [
    "title"
...
Create one plot thread inside a story: a line of causality made of scenes.
...
```json
{
  ...
  "properties": {
    "storyId": {
      "type": "string",
      "description": "A story id returned by `plot_story`."
    },
    "title": {
      "type": "string",
      "description": "Short display name of the thread; trimmed to a non-empty string."
    },
    "summary": {
      "type": "string",
      "description": "Free-form summary of the thread."
    },
    "position": {
      "type": "number",
      "description": "Ordering position within the story; lower first. Defaults to 0."
    }
  },
  "required": [
    "storyId",
...
List every plot vow with its full action history, oldest first, optionally filtered by status.
...
Plant a plot vow: a promise the story makes to the reader, expected to be paid off later.
...
Record one event on the story timeline, with the field overwrites it applies to subjects. The event's story time becomes the world state's fold point.
...
Read the story event log: every recorded world event with the subject field changes it applied, in story order, optionally filtered to one subject.
...
```json
{
  ...
  "properties": {
    "subjectId": {
      "type": "string",
      "description": "Optional subject id; only events that changed this subject are returned."
    }
  }
}
```
...
Read the whole world: every subject's folded state (last write per field wins) as of the latest recorded event, or at a given story time. Returns null before any event.
...
```json
{
  ...
  "properties": {
    "at": {
      "type": "string",
      "description": "Optional story time to fold at, as `±YYYY.MM.DD`; events strictly after it are ignored. Defaults to the latest recorded event."
    }
  }
}
```
...
Register one subject (character, location, faction, or object) in the novel's world engine, or update its baseline summary.
