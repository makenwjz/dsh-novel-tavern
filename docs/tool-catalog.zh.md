...
| `@deepseek-ai/dsh-novel-tools` | `chapter_info`、`decision_list`、`decision_record`、`novel_lint`、`vow_abandon`、`vow_advance`、`vow_list`、`vow_payoff`、`vow_plant`、`world_event`、`world_state`、`world_subject` | `ctx.tools`、`ctx.novel (mount the dsh-novel-bundle composition)` | `tool/call`、`tool/result` | - | 除非 dsh-novel-bundle 组合挂载了 ctx.novel，否则这 11 个存储工具会以 missing-runtime 错误大声失败；novel_lint 是纯文本分析，无需它也能工作。故事时间以展示形式（±YYYY.MM.DD）传递。 |
...
<a id="deepseek-aidsh-novel-tools"></a>

## `@deepseek-ai/dsh-novel-tools`

### `chapter_info`

插入或更新一个章节的知识控制账本：读者学到什么、主角学到什么，以及草稿必须隐瞒或可以暗示什么。省略的知识字段保留其先前值。
...
```json
{
  ...
  "properties": {
    ...
    "title": {
      "type": "string",
      "description": "Chapter title; trimmed to a non-empty string."
    },
    "readerKnows": {
      "type": "string",
      "description": "What the reader learns in this chapter."
    },
    "protagonistKnows": {
      "type": "string",
      "description": "What the protagonist learns in this chapter."
    },
    "mustConceal": {
      "type": "string",
      "description": "What this chapter must keep concealed."
    },
    "mayHint": {
      "type": "string",
      "description": "What this chapter may hint at without revealing."
    }
  },
  "required": [
    "number",
...
Source: [`packages/novel/novel-tools/src/index.ts`](../packages/novel/novel-tools/src/index.ts)

### `decision_list`

列出已记录的创作决策，最新的在前，含选项、选择与理由。
...
记录一个创作决策：背景、权衡的选项（含利弊），以及其开放或已决状态。
...
```json
{
  ...
  "properties": {
    "context": {
      "type": "string",
      "description": "What prompted the decision, in story terms."
    },
    "options": {
      "type": "array",
      ...
      "items": {
        ...
        "properties": {
          "label": {
            "type": "string",
            "description": "The option itself, in one short phrase."
          },
          "pros": {
            "type": "string",
          ...
          "cons": {
            "type": "string",
            "description": "Arguments against the option."
          }
        },
        "required": [
          "label"
    ...
    "chosen": {
      "type": "string",
      "description": "The chosen option label, when the decision is already made."
    },
    "rationale": {
      "type": "string",
      "description": "The rationale for the choice, when the decision is already made."
    }
  },
  "required": [
    "context",
...
对稿件片段运行风格检查：逐行报告可机械检测的行文习惯（禁用副词、叠加标点、告知式情绪、陈词滥调等）。在修改前后对草稿运行一次。
...
```json
{
  ...
  "properties": {
    "text": {
      "type": "string",
      "description": "The manuscript text to lint."
    }
  },
  "required": [
    "text"
...
```json
{
  ...
  "properties": {
    "vowId": {
      "type": "string",
      "description": "The vow id returned by `vow_plant`."
    },
    "at": {
      "type": "string",
      "description": "Story time of the action, as `±YYYY.MM.DD`."
    },
    "detail": {
      "type": "string",
      "description": "What happens at this step; trimmed to a non-empty string."
    }
  },
  "required": [
    "vowId",
...
```json
{
  ...
  "properties": {
    "status": {
      "type": "string",
      "description": "Only list vows in this status.",
      "enum": [
        "planted",
        "advanced",
        "paid_off",
...
```json
{
  ...
  "properties": {
    "title": {
      "type": "string",
      "description": "Short display name of the vow."
    },
    "promise": {
      "type": "string",
      "description": "What the story promised, in reader-visible terms."
    },
    "at": {
      "type": "string",
      "description": "Story time of the planting, as `±YYYY.MM.DD`."
    },
    "payoffTarget": {
      "type": "string",
      "description": "Where the vow is expected to resolve (a chapter, an arc, a beat)."
    },
    "note": {
      "type": "string",
      "description": "Free-form keeper notes for later continuity."
    }
  },
  "required": [
    "title",
...
在故事时间轴上记录一个事件，连同它应用到主体上的字段覆盖。事件的故事时间成为世界状态的折叠点。
...
```json
{
  ...
  "properties": {
    "storyTime": {
      "type": "string",
      "description": "Position on the story timeline, as `±YYYY.MM.DD` (for example `1200.01.01`; year 0 is the story's epoch)."
    },
    "title": {
      "type": "string",
      "description": "Short event title; trimmed to a non-empty string."
    },
    "summary": {
      "type": "string",
      "description": "Free-form description of what happens."
    },
    "changes": {
      "type": "array",
      ...
      "items": {
        ...
        "properties": {
          "subjectId": {
            "type": "string",
            "description": "A subject id returned by `world_subject`."
          },
          "field": {
            "type": "string",
            "description": "The state field being overwritten (for example `summary`, `alive`, `location`)."
          },
          "value": {
            "type": "string",
            "description": "The new value, as plain text."
          }
        },
        "required": [
          "subjectId",
...
读取截至最近记录事件的整个世界：每个主体的折叠状态（同一字段最后一次写入胜出），没有任何事件时为 null。
...
在小说世界引擎中注册一个主体（角色、地点、势力或物品），或更新其基线摘要。
...
```json
{
  ...
  "properties": {
    ...
    "name": {
      "type": "string",
      "description": "Display name; trimmed to a non-empty string."
    },
    "summary": {
      "type": "string",
      "description": "Baseline summary describing the subject as of the story's start."
    }
  },
  "required": [
    "kind",
...
除非 dsh-novel-bundle 组合挂载了 ctx.novel，否则这 11 个存储工具会以 missing-runtime 错误大声失败；novel_lint 是纯文本分析，无需它也能工作。故事时间以展示形式（±YYYY.MM.DD）传递。
