...
flowchart LR
  ...
  pkg_novel["novel"]
  svc_novel["ctx.novel<br/>Long-fiction continuity store"]
...
| `ctx.novel` | `core` | [`novel`](../packages/novel/novel) | - | `tool-novel` | - | 世界引擎、剧情伏笔账本、创作决策与章节知识控制都存放在同一个 SQLite 存储中；dsh-novel-tools 系列通过 ctx.novel 读写它。 |
