...
import * as NovelTools from '@deepseek-ai/dsh-novel-tools'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  {
    pkg: '@deepseek-ai/dsh-novel-tools',
    dir: 'novel-tools',
    source: 'packages/novel/novel-tools/src/index.ts',
    requires: ['ctx.tools', 'ctx.novel (mount the dsh-novel-bundle composition)'],
    writes: ['tool/call', 'tool/result'],
    async mount(ctx) {
      await ctx.plugin(NovelService, { root: 'tool-catalog-novel' })
      await ctx.plugin(NovelTools)
    },
    note:
      'The 11 store tools fail loud with the missing-runtime error unless the dsh-novel-bundle composition mounts ctx.novel; novel_lint is pure text analysis and works without it. Story times travel in display form (±YYYY.MM.DD).',
  },
  {
