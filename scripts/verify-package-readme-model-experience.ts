...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-novel': { kind: 'none', reason: 'Browser-side UI plugin layer; the tab only visualizes a Host-owned workspace snapshot and registers nothing model-facing.' },
  ...
  'packages/novel/novel-bundle': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
  'packages/novel/novel-api': { kind: 'none', reason: 'The Remote projection registers no prompt, tool, message, or provider request; the novel tools and bundle own the model-facing surface.' },
