...
export class FakeApiClient implements IApiClient {
  ...
  readonly tavern: IApiClient['tavern'] = {
    listWorldBooks: async () => ok({ worldbooks: [] }),
    ...
    startRoleplay: async payload => ok({ binding: { mode: 'tavern', worldbookIds: payload.worldbookIds, characterId: payload.characterId ?? null, ...(payload.characterIds === undefined ? {} : { characterIds: payload.characterIds }) } }),
    stopRoleplay: async () => ok({ binding: null }),
    lean: async () => ok({ lean: false }),
    setLean: async payload => ok({ lean: payload.lean }),
    ...
    listPromptPresets: async () => ok({ presets: [] }),
    deletePromptPreset: async () => ok({ deleted: true }),
    scoreCharacter: async () => ok({ score: { overall: 8, clarity: 9, consistency: 7, tokenEfficiency: 6, note: 'ok' } }),
    projectTree: async () => ok({ worldbooks: [], characters: [] }),
    characterImage: async () => ok({ bytesB64: 'aGVsbG8=' }),
  }
