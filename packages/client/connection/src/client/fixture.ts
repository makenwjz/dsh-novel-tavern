...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    ...
    tavern: {
      ...
      startRoleplay: request => ok(request, { binding: { mode: 'tavern', worldbookIds: request.payload.worldbookIds, characterId: request.payload.characterId ?? null, ...(request.payload.characterIds === undefined ? {} : { characterIds: request.payload.characterIds }) } }),
      stopRoleplay: request => ok(request, { binding: null }),
      lean: request => ok(request, { lean: false }),
      setLean: request => ok(request, { lean: request.payload.lean }),
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'tavern.listWorldBooks': return this.api.tavern.listWorldBooks(request)
      case 'tavern.importWorldBook': return this.api.tavern.importWorldBook(request)
      case 'tavern.deleteWorldBook': return this.api.tavern.deleteWorldBook(request)
      case 'tavern.listCharacters': return this.api.tavern.listCharacters(request)
      case 'tavern.importCharacter': return this.api.tavern.importCharacter(request)
      case 'tavern.deleteCharacter': return this.api.tavern.deleteCharacter(request)
      case 'tavern.binding': return this.api.tavern.binding(request)
      case 'tavern.setBinding': return this.api.tavern.setBinding(request)
      case 'tavern.startRoleplay': return this.api.tavern.startRoleplay(request)
      case 'tavern.stopRoleplay': return this.api.tavern.stopRoleplay(request)
      case 'tavern.lean': return this.api.tavern.lean(request)
      case 'tavern.setLean': return this.api.tavern.setLean(request)
      case 'tavern.advanceStage': return this.api.tavern.advanceStage(request)
      case 'tavern.scoreCharacter': return this.api.tavern.scoreCharacter(request)
      default:
        return Promise.reject(new Error(`fixture dispatch does not implement ${method}`))
    }
  }
