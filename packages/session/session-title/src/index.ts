...
export class SessionTitleService extends Service {
  ...
  /** Whether new human prompts schedule automatic provider generation; runtime-toggled for lean deployments. */
  ...
  /**
   * Toggle automatic provider generation. When disabled, new human prompts
   * stop scheduling provider calls and titles stay on the zero-request
   * deterministic fallback (explicit {@link SessionTitleService.refresh} and
   * {@link SessionTitleService.rename} stay available).
   * @param enabled - whether new prompts may schedule the registered provider.
   */
  ...
  private onUserMessage(session: Session, event: Extract<SessionEvent, { type: 'user/message' }>): void {
    ...
    const registration = this.registration
    if (registration !== undefined && !registration.closing && this.automatic) {
