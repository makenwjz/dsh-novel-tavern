...
describe('SessionTitleService Provider lifecycle', () => {
  ...
  it('toggles automatic provider scheduling off and back on at runtime', async () => {
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionTitleService, CONFIG)
    const generate = vi.fn(async (request: SessionTitleProviderRequest): Promise<SessionTitleProviderResult> => ({
      title: 'Generated lean-free title',
      messageSeqs: request.messages.map(message => message.seq),
    }))
    ctx.sessionTitle.register({
      id: SessionTitleProviderId('toggle'),
      automatic: 'first-prompt',
      generate,
    })

    ctx.sessionTitle.setAutomatic(false)
    const lean = ctx.sessions.create(SessionId('lean-session'))
    lean.append('turn/start', { turn: 1 })
    appendHumanPrompt(lean, 'Generate this title')
    await settle()
    appendRoute(lean)
    await settle()
    expect(generate).not.toHaveBeenCalled()
    expect(ctx.sessionTitle.get(lean)?.source.kind).toBe('fallback')

    ctx.sessionTitle.setAutomatic(true)
    const full = ctx.sessions.create(SessionId('full-session'))
    full.append('turn/start', { turn: 1 })
    appendHumanPrompt(full, 'Generate this title')
    await settle()
    appendRoute(full)
    await settle()
    expect(generate).toHaveBeenCalledTimes(1)
    expect(ctx.sessionTitle.get(full)?.source.kind).toBe('provider')
  })
})
