...
import {
  ...
  tavernSetBindingValueSchema,
  tavernSetLeanRequestSchema,
  tavernSetLeanValueSchema,
  tavernStartRoleplayRequestSchema,
  tavernStartRoleplayValueSchema,
  tavernStopRoleplayRequestSchema,
  tavernStopRoleplayValueSchema,
} from '../src/api/tavern.schema.ts'
...
describe('rpcErrorSchema', () => {
  it('accepts every code branch with its required details', () => {
    ...
    expect(rpcErrorSchema.parse({ code: 'tavern-not-available', message: 'm', details: { reason: 'bundle-not-mounted' } }).code).toBe('tavern-not-available')
    expect(rpcErrorSchema.parse({ code: 'tavern-import-invalid', message: 'm', details: { kind: 'worldbook', reason: 'not valid JSON' } }).code).toBe('tavern-import-invalid')
    expect(rpcErrorSchema.parse({ code: 'tavern-not-found', message: 'm', details: { kind: 'character', id: 'c1' } }).code).toBe('tavern-not-found')
    expect(rpcErrorSchema.parse({ code: 'tavern-still-bound', message: 'm', details: { kind: 'worldbook', id: 'w1', sessions: ['s1'] } }).code).toBe('tavern-still-bound')
...
describe('tavern domain schemas', () => {
  it('validates the store list and import pairs', () => {
    expect(tavernListWorldBooksRequestSchema.parse({})).toEqual({})
    expect(tavernListWorldBooksValueSchema.parse({ worldbooks: [] })).toEqual({ worldbooks: [] })
    expect(tavernListWorldBooksValueSchema.parse({ worldbooks: [{ id: 'w1', name: '志', entryCount: 2 }] }).worldbooks?.[0]?.name)
      .toBe('志')
    expect(() => tavernListWorldBooksValueSchema.parse({ worldbooks: [{ id: 'w1', name: '志', entryCount: -1 }] })).toThrow()
    expect(tavernImportWorldBookRequestSchema.parse({ content: '{}' }).content).toBe('{}')
    expect(tavernImportWorldBookValueSchema.parse({ worldbook: { id: 'w1', name: '志', entryCount: 2 } }).worldbook?.id).toBe('w1')
    expect(tavernDeleteWorldBookRequestSchema.parse({ id: 'w1' }).id).toBe('w1')
    expect(tavernDeleteWorldBookValueSchema.parse({ deleted: true })).toEqual({ deleted: true })
    expect(tavernListCharactersRequestSchema.parse({})).toEqual({})
    expect(tavernListCharactersValueSchema.parse({ characters: [{ id: 'c1', name: 'Aya', format: 'png' }] }).characters?.[0]?.format)
      .toBe('png')
    expect(() => tavernListCharactersValueSchema.parse({ characters: [{ id: 'c1', name: 'Aya', format: 'gif' }] })).toThrow()
    expect(tavernImportCharacterRequestSchema.parse({ fileName: 'card.png', bytesB64: 'aGk=' }).bytesB64).toBe('aGk=')
    expect(tavernImportCharacterValueSchema.parse({ character: { id: 'c1', name: 'Aya', format: 'json' } }).character?.name)
      .toBe('Aya')
    expect(tavernDeleteCharacterRequestSchema.parse({ id: 'c1' }).id).toBe('c1')
    expect(tavernDeleteCharacterValueSchema.parse({ deleted: true })).toEqual({ deleted: true })
  })

  it('validates the binding pairs', () => {
    const binding = { mode: 'tavern', worldbookIds: ['w1'], characterId: 'c1' }
    expect(tavernBindingRequestSchema.parse({ sessionId: 's1' }).sessionId).toBe('s1')
    expect(tavernBindingValueSchema.parse({ binding: null }).binding).toBeNull()
    expect(tavernBindingValueSchema.parse({ binding }).binding).toMatchObject({ mode: 'tavern' })
    expect(tavernSetBindingRequestSchema.parse({ sessionId: 's1', binding }).binding).toMatchObject({ characterId: 'c1' })
    expect(tavernSetBindingValueSchema.parse({ binding }).binding?.mode).toBe('tavern')
    expect(tavernStartRoleplayRequestSchema.parse({ sessionId: 's1', characterId: 'c1', worldbookIds: ['w1'] }).worldbookIds)
      .toEqual(['w1'])
    expect(tavernStartRoleplayValueSchema.parse({ binding }).binding?.mode).toBe('tavern')
    expect(tavernStopRoleplayRequestSchema.parse({ sessionId: 's1' }).sessionId).toBe('s1')
    expect(tavernStopRoleplayValueSchema.parse({ binding: null }).binding).toBeNull()
    expect(tavernLeanRequestSchema.parse({})).toEqual({})
    expect(tavernLeanValueSchema.parse({ lean: true }).lean).toBe(true)
    expect(tavernSetLeanRequestSchema.parse({ lean: false }).lean).toBe(false)
    expect(tavernSetLeanValueSchema.parse({ lean: true }).lean).toBe(true)
    expect(() => tavernSetLeanRequestSchema.parse({ lean: 'yes' })).toThrow()
    expect(tavernAdvanceStageRequestSchema.parse({ sessionId: 's1' }).sessionId).toBe('s1')
    expect(tavernAdvanceStageValueSchema.parse({ binding: { mode: 'novel', worldbookIds: [], characterId: null } }).binding?.mode).toBe('novel')
    expect(tavernScoreCharacterRequestSchema.parse({ id: 'c1' }).id).toBe('c1')
    expect(tavernScoreCharacterValueSchema.parse({ score: { overall: 8, clarity: 9, consistency: 7, tokenEfficiency: 6, note: 'ok' } }).score?.overall).toBe(8)
    expect(() => tavernScoreCharacterValueSchema.parse({ score: { overall: 'x' } })).toThrow()
    expect(() => tavernSetBindingRequestSchema.parse({ sessionId: 's1', binding: { mode: 'other', worldbookIds: [], characterId: null } }))
      .toThrow()
    expect(() => tavernBindingRequestSchema.parse({})).toThrow()
  })
})
