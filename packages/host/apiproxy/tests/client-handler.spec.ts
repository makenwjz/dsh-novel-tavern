...
function scriptedApi(overrides: {
  ...
  tavern?: Partial<ApiProxy['tavern']>
  ...
  return {
    ...
    tavern: {
      ...
      updateCharacter: r => ok(r, { character: { id: r.payload.id as never, name: r.payload.name ?? 'Aya', format: 'json' } }),
      updateCharacterScripts: r => ok(r, { scripts: [] }),
      saveWorldBookEntry: r => ok(r, { updated: true }),
      deleteWorldBookEntry: r => ok(r, { deleted: true }),
      deleteCharacter: r => ok(r, { deleted: true }),
      binding: r => ok(r, { binding: null }),
      setBinding: r => ok(r, { binding: r.payload.binding }),
      startRoleplay: r => ok(r, { binding: { mode: 'tavern', worldbookIds: r.payload.worldbookIds, characterId: r.payload.characterId ?? null, ...(r.payload.characterIds === undefined ? {} : { characterIds: r.payload.characterIds }) } }),
      stopRoleplay: r => ok(r, { binding: null }),
      lean: r => ok(r, { lean: false }),
      setLean: r => ok(r, { lean: r.payload.lean }),
      ...
      saveJailbreak: r => ok(r, { jailbreak: { id: 'jailbreak-j1' as never, name: r.payload.name, content: r.payload.content } }),
      listJailbreaks: r => ok(r, { jailbreaks: [] }),
      deleteJailbreak: r => ok(r, { deleted: true }),
      ...
      characterImage: r => ok(r, { bytesB64: 'aGVsbG8=' }),
      ...overrides.tavern,
    },
...
describe('tavern domain round trip', () => {
  it('carries every store and binding method over the wire form', async () => {
    const seen: { method: string; payload: unknown }[] = []
    const recorder = recorderInto(seen)
    const api = scriptedApi({
      tavern: {
        ...
        importCharacter: recorder('tavern.importCharacter', r => ok(r, { character: { id: 'character-c1' as never, name: 'Aya', format: 'json' } })),        updateCharacter: recorder('tavern.updateCharacter', r => ok(r, { character: { id: r.payload.id as never, name: r.payload.name ?? 'Aya', format: 'json' } })),
        updateCharacterScripts: recorder('tavern.updateCharacterScripts', r => ok(r, { scripts: [] })),
        saveWorldBookEntry: recorder('tavern.saveWorldBookEntry', r => ok(r, { updated: true })),
        deleteWorldBookEntry: recorder('tavern.deleteWorldBookEntry', r => ok(r, { deleted: true })),

        deleteCharacter: recorder('tavern.deleteCharacter', r => ok(r, { deleted: true })),
        binding: recorder('tavern.binding', r => ok(r, { binding: null })),
        setBinding: recorder('tavern.setBinding', r => ok(r, { binding: r.payload.binding })),
        startRoleplay: recorder('tavern.startRoleplay', r => ok(r, { binding: { mode: 'tavern', worldbookIds: r.payload.worldbookIds, characterId: r.payload.characterId ?? null, ...(r.payload.characterIds === undefined ? {} : { characterIds: r.payload.characterIds }) } })),
        stopRoleplay: recorder('tavern.stopRoleplay', r => ok(r, { binding: { mode: 'novel', worldbookIds: [], characterId: null } })),
        lean: recorder('tavern.lean', r => ok(r, { lean: true })),
        setLean: recorder('tavern.setLean', r => ok(r, { lean: r.payload.lean })),
        ...
        importChat: recorder('tavern.importChat', r => ok(r, { imported: 2 })),        saveJailbreak: recorder('tavern.saveJailbreak', r => ok(r, { jailbreak: { id: 'jailbreak-j1' as never, name: r.payload.name, content: r.payload.content } })),
        listJailbreaks: recorder('tavern.listJailbreaks', r => ok(r, { jailbreaks: [] })),
        deleteJailbreak: recorder('tavern.deleteJailbreak', r => ok(r, { deleted: true })),
    ...
    const c = client(api)
    expect(expectOk(await c.tavern.listWorldBooks({})).worldbooks?.[0]?.name).toBe('青鸾山志')
    expect(expectOk(await c.tavern.importWorldBook({ content: '{}' })).worldbook?.id).toBe('worldbook-w1')
    expect(expectOk(await c.tavern.deleteWorldBook({ id: 'worldbook-w1' as never })).deleted).toBe(true)
    expect(expectOk(await c.tavern.listCharacters({})).characters?.[0]?.format).toBe('json')
    expect(expectOk(await c.tavern.importCharacter({ fileName: 'c.png', bytesB64: 'AA==' })).character?.id).toBe('character-c1')
    expect(expectOk(await c.tavern.deleteCharacter({ id: 'character-c1' as never })).deleted).toBe(true)
    expect(expectOk(await c.tavern.binding({ sessionId: 's1' as never })).binding).toBeNull()
    const binding = { mode: 'novel' as const, worldbookIds: ['worldbook-w1' as never], characterId: null }
    expect(expectOk(await c.tavern.setBinding({ sessionId: 's1' as never, binding })).binding).toEqual(binding)
    expect(expectOk(await c.tavern.startRoleplay({ sessionId: 's1' as never, characterId: 'character-c1' as never, worldbookIds: ['worldbook-w1' as never] })).binding)
      .toEqual({ mode: 'tavern', worldbookIds: ['worldbook-w1'], characterId: 'character-c1' })
    expect(expectOk(await c.tavern.stopRoleplay({ sessionId: 's1' as never })).binding).toEqual({ mode: 'novel', worldbookIds: [], characterId: null })
    expect(expectOk(await c.tavern.lean({})).lean).toBe(true)
    expect(expectOk(await c.tavern.setLean({ lean: false })).lean).toBe(false)
    expect(expectOk(await c.tavern.advanceStage({ sessionId: 's1' as never })).binding.stage).toBe(1)
    expect(expectOk(await c.tavern.scoreCharacter({ id: 'character-c1' as never })).score.overall).toBe(8)
    expect(seen.map(entry => entry.method)).toEqual([
      'tavern.listWorldBooks', 'tavern.importWorldBook', 'tavern.deleteWorldBook', 'tavern.listCharacters',
      'tavern.importCharacter', 'tavern.deleteCharacter', 'tavern.binding', 'tavern.setBinding',
      'tavern.startRoleplay', 'tavern.stopRoleplay', 'tavern.lean', 'tavern.setLean', 'tavern.advanceStage',
      'tavern.scoreCharacter',
    ])
  })

  it('rejects a binding-less stop request payload at the handler schema', async () => {
    const response = await client(scriptedApi()).tavern.stopRoleplay({} as never)
    expect(response.result.ok).toBe(false)
    if (!response.result.ok) expect(response.result.error.code).toBe('bad-request')
  })
})
