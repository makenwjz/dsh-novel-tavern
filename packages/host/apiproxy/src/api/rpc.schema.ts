...
export const rpcErrorSchema: z.ZodType<RpcError> = z.discriminatedUnion('code', [
  ...
  z.object({ code: z.literal('tavern-not-available'), message: z.string(), details: z.object({ reason: z.string() }) }),
  z.object({ code: z.literal('tavern-import-invalid'), message: z.string(), details: z.object({ kind: z.enum(['worldbook', 'character']), reason: z.string() }) }),
  z.object({ code: z.literal('tavern-not-found'), message: z.string(), details: z.object({ kind: z.enum(['worldbook', 'character']), id: z.string() }) }),
  z.object({ code: z.literal('tavern-still-bound'), message: z.string(), details: z.object({ kind: z.enum(['worldbook', 'character']), id: z.string(), sessions: z.array(z.string()) }) }),
