...
export interface RpcErrorDetailsMap {
  ...
  /** The tavern store is not composed; the message names the bundle to mount. */
  'tavern-not-available': { reason: string }
  /** An import was refused; the message is the parser's rejection text. */
  'tavern-import-invalid': { kind: 'worldbook' | 'character'; reason: string }
  /** A delete named no stored object. */
  'tavern-not-found': { kind: 'worldbook' | 'character'; id: string }
  /** A delete was refused while a session's binding references the object. */
  'tavern-still-bound': { kind: 'worldbook' | 'character'; id: string; sessions: string[] }
  'internal': {}
}
