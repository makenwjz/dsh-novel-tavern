...
// Type-only: resolves `ctx.get('tavern')` and the tavern wire brands.
import type {} from '@deepseek-ai/dsh-tavern'
import type { TavernBindingData } from '@deepseek-ai/dsh-tavern/types'
// Type-only: resolves `ctx.get('sessionTitle')` for the lean title toggle.
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Resolve the tavern store the deployment composes (bundle-level, process-wide). */
  function tavernStore(): NonNullable<ReturnType<typeof ctx.get<'tavern'>>> | { error: RpcError } {
    const tavern = ctx.get('tavern')
    if (tavern === undefined) {
      return {
        error: {
          code: 'tavern-not-available',
          message: 'the tavern roleplay store is absent: mount @deepseek-ai/dsh-novel-bundle in the profile',
          details: { reason: 'bundle-not-mounted' },
        },
      }
    }
    return tavern
  ...
  /** Map one store rejection (import or delete) to its stable wire code. */
  ...
  /** Append a validated binding event, failing loud before anything is logged. */
  function applyBinding(request: RpcRequest<{ sessionId: SessionId }>, binding: TavernBindingData):
  RpcResponse<{ binding: TavernBindingData }> {
    ...
    if (session === undefined) {
      return err(request, {
        ...
        message: `session "${request.payload.sessionId}" does not exist`,
    ...
    for (const id of binding.worldbookIds) {
      try {
        ...
        return err(request, {
          code: 'tavern-not-found',
          message: error instanceof Error ? error.message : String(error),
          details: { kind: 'worldbook', id },
        })
      }
    }
    if (binding.characterId !== null) {
      try {
        tavern.characterProfile(binding.characterId)
      } catch (error: unknown) {
        return err(request, {
          code: 'tavern-not-found',
          message: error instanceof Error ? error.message : String(error),
          details: { kind: 'character', id: binding.characterId },
        })
      }
    }
    if (binding.characterIds !== undefined) {
      for (const id of binding.characterIds) {
        try {
          tavern.characterProfile(id)
        } catch (error: unknown) {
          return err(request, {
            code: 'tavern-not-found',
            message: error instanceof Error ? error.message : String(error),
            details: { kind: 'character', id },
          })
        }
      }
    }
    session.append('tavern/binding', binding)
  ...
  return {
    ...
    tavern: {
      // The store is process-wide; binding state is per-session and durable
      // in the session log (`tavern/binding` events), so every read folds the
      // log and every write appends one event after validating references.
      async listWorldBooks(request) {
        const tavern = tavernStore()
        if ('error' in tavern) return err(request, tavern.error)
        try {
          return ok(request, { worldbooks: tavern.listWorldBooks() })
        } catch (error: unknown) {
          return tavernStoreError(request, error, 'worldbook', 'import')
        }
      },
      ...
      async importWorldBook(request) {
        ...
        if ('error' in tavern) return err(request, tavern.error)
        try {
          return ok(request, { worldbook: tavern.importWorldBook(request.payload.content) })
        } catch (error: unknown) {
          return tavernStoreError(request, error, 'worldbook', 'import')
        }
      },
      ...
      async deleteWorldBook(request) {
        ...
        if ('error' in tavern) return err(request, tavern.error)
        try {
          tavern.deleteWorldBook(request.payload.id)
          return ok(request, { deleted: true as const })
        } catch (error: unknown) {
          return tavernStoreError(request, error, 'worldbook', 'delete')
        }
      },
      ...
      async listCharacters(request) {
        ...
        if ('error' in tavern) return err(request, tavern.error)
        try {
          return ok(request, { characters: tavern.listCharacters() })
        } catch (error: unknown) {
          return tavernStoreError(request, error, 'character', 'import')
        }
      },
      ...
      async importCharacter(request) {
        ...
        try {
          return ok(request, {
            character: tavern.importCharacter(request.payload.fileName, Buffer.from(request.payload.bytesB64, 'base64')),
          })
        } catch (error: unknown) {
          return tavernStoreError(request, error, 'character', 'import')
        }
      ...
      async deleteCharacter(request) {
        ...
        if ('error' in tavern) return err(request, tavern.error)
        try {
          tavern.deleteCharacter(request.payload.id)
          return ok(request, { deleted: true as const })        } catch (error: unknown) {
          return tavernStoreError(request, error, 'character', 'delete')
        }
      },
      ...
      async binding(request) {
        ...
        return ok(request, { binding: tavern.bindingOf(session.id) })
      },

      async setBinding(request) {
        return applyBinding(request, request.payload.binding)
      },
      ...
      async stopRoleplay(request) {
        ...
        const current = tavern.bindingOf(session.id)
        if (current === null) return ok(request, { binding: null })
        const binding: TavernBindingData = {
          mode: 'novel',
      ...
      async lean(request) {
        const tavern = tavernStore()
        if ('error' in tavern) return err(request, tavern.error)
        return ok(request, { lean: tavern.lean })
      },

      async setLean(request) {
        const tavern = tavernStore()
        if ('error' in tavern) return err(request, tavern.error)
        tavern.setLean(request.payload.lean)
        // Lean deployments cut the fire-and-forget title call: the fallback
        // title stays zero-request when the title service is composed.
        ctx.get('sessionTitle')?.setAutomatic(!request.payload.lean)
        return ok(request, { lean: tavern.lean })
      },
      ...
      async advanceStage(request) {
        ...
        if (tavern.bindingOf(session.id) === null) {
          return err(request, {
            code: 'bad-request',
            message: 'the session has no tavern binding to advance',
            details: { issues: [] },
          })
        }
      ...
      async scoreCharacter(request) {
        ...
        if ('error' in tavern) return err(request, tavern.error)
        try {
          return ok(request, { score: await tavern.scoreCharacter(request.payload.id) })
        } catch (error: unknown) {
          const message = error instanceof Error ? error.message : String(error)
          return err(request, { code: 'internal', message, details: {} })
        }
      ...
      async projectTree(request) {
        const tavern = tavernStore()
        if ('error' in tavern) return err(request, tavern.error)
        try {
          const tree = tavern.projectTree()
          return ok(request, {
            worldbooks: tree.worldbooks.map(book => ({
              id: book.id,
              name: book.name,
            ...
            characters: tree.characters.map(character => ({
              id: character.id,
              name: character.name,
      ...
      async characterImage(request) {
        const tavern = tavernStore()
        if ('error' in tavern) return err(request, tavern.error)
        try {
          return ok(request, { bytesB64: tavern.characterImage(request.payload.id) })
        } catch (error: unknown) {
          const message = error instanceof Error ? error.message : String(error)
          return err(request, { code: 'internal', message, details: {} })
        }
