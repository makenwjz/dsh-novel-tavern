...
function fakeInternals() {
  ...
  return {
    internals, files, dirs, memories, kills, execs,
...
describe('macOS process inspector', () => {
  ...
  describe('Windows process inspector', () => {
    it('derives process trees from the CIM table, fences signals by identity, and kills groups via taskkill', () => {
      const fake = fakeInternals()
      fake.setPs([
        '100|1|638987040000000000',
        '101|100|638987040000000001',
        '102|101|638987040000000002',
        '103|99|638987040000000003',
        'malformed',
      ].join('\n'))
      const inspector = createProcessInspector('win32', 'x64', fake.internals)
      expect(inspector.foregroundPgid(10)).toBeUndefined()
      expect(inspector.isStdinWaiting(55)).toBe(false)
      expect(inspector.processTree(100)).toEqual([
        { pid: 102, started: '638987040000000002' },
        { pid: 101, started: '638987040000000001' },
        { pid: 100, started: '638987040000000000' },
      ])
      expect(inspector.processTree(99)).toEqual([])
      expect(inspector.processSession(10)).toEqual([])
      expect(inspector.isAlive({ pid: 101, started: '638987040000000001' })).toBe(true)
      expect(inspector.isAlive({ pid: 101, started: 'stale' })).toBe(false)
      inspector.signalProcess({ pid: 101, started: '638987040000000001' }, 'SIGKILL')
      inspector.signalProcess({ pid: 102, started: 'stale' }, 'SIGTERM')
      inspector.signalGroup(100, 'SIGTERM')
      // Every tree/alive read walks the CIM table (six reads above), a live
      // identity fences the process kill, a stale identity skips it, and a
      // group signal becomes a whole-tree taskkill.
      expect(fake.execs.filter(([file]) => file === 'powershell.exe')).toHaveLength(6)
      expect(fake.execs.findLast(([file]) => file === 'taskkill'))
        .toEqual(['taskkill', ['/T', '/F', '/PID', '100']])
      expect(fake.execs.find(([file, args]) => file === 'taskkill' && args.includes('/PID')))
        .toEqual(['taskkill', ['/F', '/PID', '101']])
    })

    it('fails closed when powershell is unavailable', () => {
      const fake = fakeInternals()
      fake.internals.exec = () => { throw new Error('powershell gone') }
      const inspector = createProcessInspector('win32', 'x64', fake.internals)
