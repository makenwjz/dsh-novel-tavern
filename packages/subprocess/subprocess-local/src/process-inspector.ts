...
/**
 * One Windows process row: `Win32_Process.ProcessId`, `ParentProcessId`, and
 * `CreationDate.Ticks` (the last is the stable start identity — ticks do not
 * localize, unlike a rendered DateTime).
 */
interface WinProcessRow extends ProcessTreeEntry {}

/** Windows process table via `Get-CimInstance Win32_Process` (Win8.1+). */
function windowsProcessTable(internals: ProcessInspectorInternals): WinProcessRow[] {
  const script = 'Get-CimInstance Win32_Process | ForEach-Object { \"{0}|{1}|{2}\" -f $_.ProcessId,$_.ParentProcessId,$_.CreationDate.Ticks }'
  let output: string
  try {
    output = internals.exec('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', script])
  } catch (_powershellUnavailable) {
    return []
  }
  return output.split('\n').flatMap((line) => {
    const [pidText, parentText, started] = line.trim().split('|')
    if (pidText === undefined || parentText === undefined || started === undefined) return []
...
/**
 * Windows process inspector. Windows has no POSIX process groups or sessions:
 * foreground group reads are unavailable, group signals become tree kills via
 * `taskkill /T`, and a session is the terminal-server logon session nothing in
 * the subprocess stack keys by. Readiness probing degrades to "not waiting".
 */
class WindowsProcessInspector implements ProcessInspector {
  ...
  processTree(rootPid: number): ProcessIdentity[] {
    return processTree(windowsProcessTable(this.internals), rootPid)
  }
  ...
  isAlive(identity: ProcessIdentity): boolean {
    return windowsProcessTable(this.internals).some(
      entry => entry.pid === identity.pid && entry.started === identity.started,
    )
  }

  signalGroup(rootPid: number, _signal: SubprocessTerminalSignal): void {
    this.internals.exec('taskkill', ['/T', '/F', '/PID', String(rootPid)])
  }

  signalProcess(identity: ProcessIdentity, _signal: 'SIGTERM' | 'SIGKILL'): void {
    if (!this.isAlive(identity)) return
    this.internals.exec('taskkill', ['/F', '/PID', String(identity.pid)])
  }
}
