...
// Fall back to the real rename so existing cases keep working; the retry case
// overrides the mock for its own first call.
...
describe('writeFileAtomic', () => {
  ...
  // so this rename-to-symlink contract is exercised on POSIX only.
  ...
  it('retries a transient Windows sharing violation before committing the rename', async () => {
    ...
    renameMock.mockRejectedValueOnce(Object.assign(new Error('EPERM'), { code: 'EPERM' }))
    await writeFileAtomic(target, 'new', { mode: 0o600 })
