...
/** Whether a rename failed because a reader or antivirus briefly holds the target (Windows). */
function isTransientRenameError(error: unknown): boolean {
  const code = (error as NodeJS.ErrnoException | null)?.code
  return code === 'EPERM' || code === 'EBUSY'
}

const RENAME_RETRY_INITIAL_MS = 25
const RENAME_RETRY_ATTEMPTS = 8

/**
 * Commit the temp over the target with short backoff on transient Windows
 * sharing violations (`EPERM`/`EBUSY`): a settings watcher or antivirus can
 * hold the target for a few ticks, and a one-shot rename would tear down the
 * write. Every other failure (including a permanent `EPERM`, e.g. the target
 * is a directory) rethrows immediately after the bounded retries.
 * @param temp - the prepared random-suffix sibling.
 * @param filename - final path receiving the content.
 */
async function commitRename(temp: string, filename: string): Promise<void> {
  for (let attempt = 0; ; attempt++) {
    try {
      await rename(temp, filename)
      return
    } catch (error) {
      if (!isTransientRenameError(error) || attempt >= RENAME_RETRY_ATTEMPTS) throw error
      await new Promise(resolve => setTimeout(resolve, RENAME_RETRY_INITIAL_MS * (attempt + 1)))
    }
  }
}
...
export async function writeFileAtomic(filename: string, content: string, options: WriteFileAtomicOptions): Promise<void> {
  ...
  try {
    ...
    await commitRename(temp, filename)
  } catch (error) {
