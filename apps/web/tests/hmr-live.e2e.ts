...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  ...
  try {
    ...
    // On Windows pnpm resolves through `pnpm.cmd`, which spawn needs wrapped in
    // `cmd.exe /c`; a bare `pnpm`/`pnpm.cmd` spawn fails ENOENT/EINVAL.
    ...
    // First paint rides the Vite dev server's cold transform; Windows cold
    // starts are slower than the POSIX CI lane, so the wait is generous.
    await page.getByText(oldText, { exact: true }).waitFor({ timeout: 90_000 })
