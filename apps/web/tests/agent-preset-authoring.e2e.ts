...
describe('web e2e: agent-preset authoring is a host-side copy', () => {
  ...
  function withPresetRoot(snapshot: string): string {
    // String values inside quotes escape backslashes; bare `code:`/`text:` rows
    // do not — match both spellings.
    const roots = [userRoot, userRoot.replace(/\\/g, '\\\\')]
    return snapshot.split('\n').map((line) => {
      // The root rides either separator on Windows (`\`), always `/` on POSIX.
      let rootStart = -1
      let rootLength = 0
      for (const root of roots) {
        for (const separator of ['/', '\\']) {
          const at = line.indexOf(`${root}${separator}`)
          if (at !== -1) {
            rootStart = at
      ...
      const rest = line.slice(rootStart + rootLength).replace(/^[\\/]/, '')
      return `${line.slice(0, pathStart)}{{presetRoot}}/${rest}`
    }).join('\n')
  }
