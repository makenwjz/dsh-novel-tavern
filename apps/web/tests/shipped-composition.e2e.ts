...
// The shell stack is platform-gated: POSIX ships bash, Windows ships pwsh
// (the base bundles mount exactly one — bash has no Windows runner). The
// catalog is sorted, so each platform's shell slot sits at its alphabetic
// position (`bash` after `ask_user_question`, `pwsh` after `list_agents`).
const EXPECTED_TOOLS: readonly string[] = process.platform === 'win32'
  ? [
    'ask_user_question',
    'create_goal',
    'edit',
...
it('lets a preset producer reach the background-job registry', async () => {
  ...
  try {
    ...
    // The shell tool (`tool-bash` on POSIX, `tool-pwsh` on Windows) is a preset
    // row and `tasks` is a host registry; the producer resolves it with
    // `ctx.get`, so a registry hidden behind a preset realm fails here — with
    // every task control still listed in the catalog above.
    const started = await ctx.tools.execute({
      signal,
      callId: CallId('shipped-shell-background'),
      name: SHELL_TOOL,
      arguments: {
        command: SHELL_TOOL === 'pwsh' ? 'Write-Output SHIPPED_BACKGROUND_OK' : 'printf SHIPPED_BACKGROUND_OK',
