...
// with a usable `pwsh`. The shipped base/web bundles gate shell stacks by
// platform — Windows mounts the pwsh executor and tool, POSIX hosts mount
// bash and no pwsh — so this parity lane is Windows-only.
...
// The overlay only disables the shipped permission row (the pwsh executor and
// tool are already shipped on Windows); a host without a usable `pwsh` cannot
// boot that stack, so the lane self-skips, mirroring the pwshOnly ACP
// scenarios. The probe follows the executor's own
...
describe.skipIf(MODE === 'record' || process.platform !== 'win32' || !HAS_PWSH)('web e2e: pwsh calls use the bash terminal-card layout', () => {
