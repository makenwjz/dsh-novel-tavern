...
describe('web e2e: navigation & panes over a rich seeded session', () => {
  ...
  // The seeded call is bash, and Windows ships no bash runner (the base bundles
  // gate bash to POSIX; pwsh is the Windows twin) — so this layout-stability
  // lane is POSIX-only. Windows gets the same terminal-card layout via the
  // pwsh-terminal lane.
  ...
  // Windows ships no bash runner (the bundles gate bash to POSIX; pwsh is the
  // Windows twin), so the bash terminal card is POSIX-only here; pwsh-terminal
  // covers the Windows card.
