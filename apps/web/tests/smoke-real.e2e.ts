...
// The CLI's tsx source launch resolves tsconfig `paths` through tsx's own
// resolver, which splits on spaces — a repo path containing a space (e.g.
// `C:\bd\my subject\...`) makes `dsh web` exit before printing its URL line.
// CI paths have no spaces; the scaffold-based lanes (vendored Loader, no tsx)
// are unaffected, so this real-CLI lane self-skips on a spaced repo path.
const REPO_PATH_HAS_SPACE = REPO_ROOT.includes(' ')

describe.skipIf(REPO_PATH_HAS_SPACE)('dsh web keyless CLI smoke', () => {
