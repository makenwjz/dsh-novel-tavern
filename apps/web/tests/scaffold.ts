...
export function realizeSeedFixture(scaffold: WebScaffold, fixtureText: string, id: string): string {
  const realized = fixtureText
    ...
    // The cwd token sits inside JSON string values; a Windows path needs its
    // backslashes JSON-escaped or the first line (and every later tool line)
    // fails to parse.
    .split('{{cwd}}').join(JSON.stringify(scaffold.workspaceCwd).slice(1, -1))
...
function normalizeAria(snapshot: string, workspaceCwd: string): string {
  ...
  // A session cwd that lives under the workspace (the workspace-aware flow runs
  // sessions in <workspaceCwd>/workspace) renders its paths with that prefix,
  // which is more specific than the workspace root; replacing it first keeps
  // `{{cwd}}/nav-a.md` stable whether or not a scenario adds the subdirectory.
  // Aria snapshots escape backslashes and quotes in their string values, so a
  // Windows path must match its escaped spelling.
  const escaped = (path: string): string => path.replace(/\\/g, '\\\\')
  const sessionCwd = `${workspaceCwd}/workspace`
  return snapshot
    .split(escaped(sessionCwd)).join('{{cwd}}')
    .split(escaped(workspaceCwd)).join('{{cwd}}')
