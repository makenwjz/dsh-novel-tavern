...
describe('web e2e: Models settings page configures a dormant provider', () => {
  ...
  it('confirms an identified provider deletion before removing its profile and key', async () => {
    ...
    await deleteDialog.getByRole('button', { name: '取消', exact: true }).click()
    // Wait for the cancel's dialog to finish closing: a second open while the
    // mask is still tearing down leaves the re-open click intercepted and the
    // deletion never sent.
    await expect.poll(() => page.getByRole('dialog', { name: '删除 minimax-cn？' }).count(), { timeout: 10_000 }).toBe(0)
    expect(await readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8')).toContain('minimax-cn:')
    await settingsDialog.getByRole('button', { name: '删除 minimax-cn', exact: true }).click()
    const confirm = page.getByRole('dialog', { name: '删除 minimax-cn？' })
    await confirm.waitFor({ timeout: 10_000 })
    await confirm.getByRole('button', { name: '删除 minimax-cn', exact: true }).click()
    await expect.poll(
